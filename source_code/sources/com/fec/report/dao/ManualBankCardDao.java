...
public class ManualBankCardDao extends AbstractDao<ManualBankCard, Long> {
    public static final String TABLENAME = "MANUAL_BANK_CARD";
    private Query<ManualBankCard> manualInfo_ManualBankCardListQuery;
    ...
    public ManualBankCardDao(DaoConfig config) {
        super(config);
    }

    public ManualBankCardDao(DaoConfig config, DaoSession daoSession) {
        super(config, daoSession);
    }

    public static void createTable(Database db, boolean ifNotExists) {
        db.execSQL("CREATE TABLE " + (ifNotExists ? "IF NOT EXISTS " : "") + "\"MANUAL_BANK_CARD\" (\"LOCAL_ID\" INTEGER PRIMARY KEY AUTOINCREMENT ,\"ID\" TEXT,\"LOCAL_FK_IR\" INTEGER,\"BANK\" TEXT,\"ACCOUNT\" TEXT,\"PHONE\" TEXT,\"PASSPORT\" TEXT,\"NOTE\" TEXT);");
    }

    public static void dropTable(Database db, boolean ifExists) {
        db.execSQL("DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "\"MANUAL_BANK_CARD\"");
    }

    protected final void bindValues(DatabaseStatement stmt, ManualBankCard entity) {
        stmt.clearBindings();
    ...
    protected final void bindValues(SQLiteStatement stmt, ManualBankCard entity) {
        stmt.clearBindings();
        Long local_id = entity.getLocal_id();
        if (local_id != null) {
            stmt.bindLong(1, local_id.longValue());
    ...
    public ManualBankCard readEntity(Cursor cursor, int offset) {
        String str = null;
        ...
        return new ManualBankCard(valueOf, string, valueOf2, string2, string3, string4, string5, str);
    }

    public void readEntity(Cursor cursor, ManualBankCard entity, int offset) {
        String str = null;
    ...
    protected final Long updateKeyAfterInsert(ManualBankCard entity, long rowId) {
        entity.setLocal_id(Long.valueOf(rowId));
        return Long.valueOf(rowId);
    }

    public Long getKey(ManualBankCard entity) {
        if (entity != null) {
    ...
    public boolean hasKey(ManualBankCard entity) {
    ...
    public List<ManualBankCard> _queryManualInfo_ManualBankCardList(Long local_fkIr) {
        synchronized (this) {
            if (this.manualInfo_ManualBankCardListQuery == null) {
                QueryBuilder<ManualBankCard> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.Local_fkIr.eq(null), new WhereCondition[0]);
                this.manualInfo_ManualBankCardListQuery = queryBuilder.build();
            }
        }
        Query<ManualBankCard> query = this.manualInfo_ManualBankCardListQuery.forCurrentThread();
